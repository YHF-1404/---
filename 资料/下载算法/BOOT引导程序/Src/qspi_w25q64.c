/****
   ...
   *  @description
	*
	*	实验平台：反客STM32H750VBT6核心板 （型号：FK750M3-VBT6）
...
>>>>> 重要说明：
	...
	*
	*	6.如果使用MDMA进行读取，可以达到 55.56M字节/S，使用内存映射模式的话，可以达到 59.7M/s
	*
...
*			    5.在本例程中，使用的是库函数进行直接读写，keil版本5.30，编译器AC6.14，编译等级Oz image size，读取速度为 6.7M字节/S ，
